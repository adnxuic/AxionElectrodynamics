...
\subsection{Topological magneto electric effect}

we have seen that topological insulators are a class fascinating class of materials whose dynamic is characterised by the fact that $\theta = \uppi$.
...
First, we place a topological insulator in a uniform magnetic field directed along the z axis. Because $\theta$ undergoes a discontinuity at the surface of the topological insulator, charge will be induced on the surface:
\begin{equation}
...
Alternatively, we can turn on an electric field lies tangential to the interface, we place a topological insulator in uniform electric field directed along the y axis. Similarly, because $\theta$ undergoes a discontinuity at the surface of the topological insulator, a surface current $\bm{K}$ will therefore be induced:
\begin{equation}
\int \bm{K} \cdot \mathrm{d}l = \int \nabla \times \bm{B} \mathrm{d}s = -\int \frac{\alpha}{\uppi} \nabla \theta \times \bm{E} \mathrm{d}s
\end{equation}
Simplistically, we only consider the surface current on upper interface, so
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The creation of a two-dimensional current which lies perpendicular to an applied electric field is called the Hall effect the coefficient of proportionality is known as the Hall conductivity:
\begin{equation}
